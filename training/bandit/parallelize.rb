...
def usage
  STDERR.write "Usage: "
  STDERR.write "ruby parallelize.rb -c <bandit.ini> [-e <epochs=10>] [--randomize/-z] [--reshard/-y] -s <#shards|0> [-p <at once=9999>] -i <input> -r <refs> [--qsub/-q] [--bandit_binary <path to bandit binary>] [-l \"l2 select_k 100000\"] [--extra_qsub \"-l virtual_free=24G\"]\n"
  exit 1
end

opts = Trollop::options do
  opt :config, "bandit config file", :type => :string
  opt :epochs, "number of epochs", :type => :int, :default => 100
  opt :lplp_args, "arguments for lplp.rb", :type => :string, :default => "l2 select_k 10000"
...
if not opts[:bandit_binary]
  bandit_bin = "#{bandit_dir}/bandit_mt"
else
  bandit_bin = opts[:bandit_binary]
end
ruby       = '/usr/bin/ruby'
lplp_rb    = "#{bandit_dir}/../dtrain/lplp.rb"
